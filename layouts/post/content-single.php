...
<div class="els-blog-single">
  <div id="post-<?php the_ID(); ?>">
            ...
            foreach ( $ids as $id ) {
              ...
              if( ($count === 1) || (($count % 2) === 1) ) {
                ...
                if( ($count_img === 1) || (($row_number % 2) === 1 && ($row_number === $row_img)) ) {
                  echo '<li class="box col-lg-12 col-md-12 col-sm-12 col-12">';
                  echo '<a href='. esc_url($attachment[0]) .'><img src="'. esc_url($attachment[0]) .'" alt="'. esc_attr($alt) .'" /></a></li>';
                } else {
                  if( ($row_img === 1) || (($row_img % 2) === 1) ) {
                      echo '<li class="box col-lg-8 col-md-8 col-sm-8 col-8">';
                      if(class_exists('Aq_Resize')) {
                        $actual_img = aq_resize( $attachment[0], '575', '320', true );
                      } else {
                        $actual_img = $attachment[0];
                      }
                      echo '<a href='. esc_url($attachment[0]) .'><img src="'. esc_url($actual_img) .'" alt="'. esc_attr($alt) .'" /></a></li>';
                    } else {
                      if(class_exists('Aq_Resize')) {
                        $actual_img = aq_resize( $attachment[0], '380', '230', true );
                      } else {
                        $actual_img = $attachment[0];
                      }
                      echo '<li class="box col-lg-5 col-md-5 col-sm-5 col-5">';
                      echo '<a href='. esc_url($attachment[0]) .'><img src="'. esc_url($actual_img) .'" alt="'. esc_attr($alt) .'" /></a></li>';
                    }
                }
                ...
                if( ($row_img === 1) || (($row_img % 2) === 1) ) {
                  echo '<li class="box col-lg-4 col-md-4 col-sm-4 col-4">';
                  if(class_exists('Aq_Resize')) {
                    $actual_img = aq_resize( $attachment[0], '250', '320', true );
                  } else {
                    $actual_img = $attachment[0];
                  }
                  echo '<a href='. esc_url($attachment[0]) .'><img src="'. esc_url($actual_img) .'" alt="'. esc_attr($alt) .'" /></a></li>';
                  ...
                  if(class_exists('Aq_Resize')) {
                    $actual_img = aq_resize( $attachment[0], '445', '230', true );
                  } else {
                    $actual_img = $attachment[0];
                  }
                  echo '<li class="box col-lg-7 col-md-7 col-sm-7 col-7">';
                  echo '<a href='. esc_url($attachment[0]) .'><img src="'. esc_url($actual_img) .'" alt="'. esc_attr($alt) .'" /></a></li>';
                }
              }
    ...
    if (!in_array('tag', $elsey_single_metas_hide) && $elsey_single_share_option) {
      $single_tags_col_class  = 'col-lg-5 col-md-6 col-sm-6 col-12';
      $single_share_col_class = 'col-lg-7 col-md-6 col-sm-6 col-12';
    } else if (!in_array('tag', $elsey_single_metas_hide) || $elsey_single_share_option) {
      $single_tags_col_class  = 'col-lg-12 col-md-12 col-sm-12 col-12';
      $single_share_col_class = 'col-lg-12 col-md-12 col-sm-12 col-12';
    }
    if ( !in_array('tag', $elsey_single_metas_hide) || $elsey_single_share_option) { ?>
